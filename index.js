...
function getCarInfoById(inventory, id) {
  for (let I = 0; I < inventory.length; ++I) {
    if (inventory[I].id === id) {
      const firstID = inventory[I];
      return `The car is a ${firstID.car_make} ${firstID.car_model}`;
    }
  }
} /*w3schools researched*/
 ...
 */ function sortCarInventory(inventory) {
  var swapp;
  var n = inventory.length - 1;
  var x = inventory;
  do {
    let swapp = false;
    for (var i = 0; i < n; i++) {
      if (x[i].car_model > x[i + 1].car_model) {
        var temp = x[i];
        x[i] = x[i + 1];
        x[i + 1] = temp;
        swapp = true;
 ...
 */
function getOlderCars(inventory, old) {
  let years = [];
  for (let I = 0; I < inventory.length; ++I) {
    if (inventory[I].car_year <= old) years.push(inventory[I]);
  }
  return years;
}
